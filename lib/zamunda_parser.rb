class ZamundaParser
  def self.get_links(serie,season,episode)
    ...
    # page_counter = 0;
    # begin
      # next_page = website+"browse.php?c33=1&c7=1&search="+serie+"&incldead=1&field=name&page="+page_counter.to_s
      # agent = Mechanize.new
      # zamunda = agent.get(next_page)
      # login = zamunda.form_with(:name => "login")
      # return zamunda
...
private
  ...
  def get_subs_link(serie_page)
    ...
    login.field_with(:name => "username").value = "watch1tteam"
    login.field_with(:password => "password").value = "PowerPassword1"
    zamunda_body = login.submit.body
    nokogiri_doc = Nokogiri::HTML(zamunda_body)
